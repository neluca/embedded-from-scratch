...
> M0 仅实现 2 位优先级 (0, 64, 128, 192 = 0x00, 0x40, 0x80, 0xC0)。
> M3/M4 可配置 3-8 位 (最多 256 级)。数值越小 = 优先级越高。
...
```
场景: 低优先级 ISR 正在执行, 高优先级中断到达

1. 硬件自动保存当前 ISR 的上下文 (R0-R3, R12, LR, PC, xPSR)
2. 执行高优先级 ISR
3. 高优先级 ISR 返回 (BX LR)
4. 硬件自动恢复低优先级 ISR 的上下文 → 继续执行
```

抢占规则：
- **高优先级可以抢占低优先级**（数值小的抢占数值大的）
- **同优先级不能抢占**（先来先服务）
- **HardFault 和 NMI 可以抢占一切**（负优先级，数值上最高）

> M0 **没有** sub-priority（子优先级）——同优先级中断按到达顺序处理，不会互相抢占。

### 尾链 (Tail-Chaining)

如果 CPU 从异常返回时发现还有 pending 的中断，**跳过完整的压栈/出栈**，直接进入下一个 ISR：

```
正常流程: ISR1 结束 → 出栈 8 个字 → 压栈 8 个字 → ISR2 开始
尾链优化: ISR1 结束 → (跳过出/入栈) → ISR2 开始
...
如果正在为低优先级中断压栈时，更高优先级的中断到达：

```
1. 硬件正在为 ISR_LOW 压栈 R0-R3...
2. ISR_HIGH 到达
3. 硬件压完栈后 → 不进入 ISR_LOW → 直接进入 ISR_HIGH
4. ISR_HIGH 完成后 → 进入 ISR_LOW（它还在 pending）
```
...
问题场景:
  Task_LOW  持有 Mutex
  Task_HIGH 等待 Mutex (被阻塞!)
  Task_MID  抢占 Task_LOW (Mutex 持有者被延迟!)
  → Task_HIGH 的优先级被 Task_MID 反转了
```

解决方案：
- **FreeRTOS 互斥锁**: 自动临时提升持有者优先级 (priority inheritance)
- **临界区最小化**: 减少关中断的时间
- **中断优先级分层**: 将关键 ISR 设为高优先级
